...
def skewed_triangle(x: float, L: float, m: int,
amplitude: float, phase: float, offset: float) -> float:
    """
...
def skewed_triangle_wave(duration: float, sampling_rate: int, L: float, m: int, a: float, p: float, b: float) -> np.ndarray:
    """
...
def main():
    ...
    if True:
        ...
        data_analysis.plot_signal(sig_sum, time, rand_args.sampling_rate, save_path=Path("python_sum_combined.png"))
        data_analysis.plot_individual_oscillators(signal_matrix, time, save_path=Path("python_sum_individual.png"))
        plt.show()
